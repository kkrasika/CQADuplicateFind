...
import pandas as pd
import gc
from DataSetUtil import get_df_from_csv_file, num_of_records_for_domain, get_part_df_from_csv_file, review_to_wordlist, get_df_from_csv_files_combined
...
def main():
    ...
    for commonModel in [True]:
        ...
        for rerank in [True]:
            ...
            train_x, train_y, train_domain_list, valid_x, valid_y, valid_domain_list, embedding_meta_data_full = get_doc2vec_vectors_train_valid_split(
                df_combined)
            model_path = train_model(train_x, train_y, train_domain_list, embedding_meta_data_full, 'common-adapted')
            ...
            preds, accuracy = evaluate_model(siamese_lstm_model, valid_x, valid_y, valid_domain_list)
            ...
            for a in range(len(fileNameList)):
                ...
                train_x, train_y, train_domain_list, valid_x, valid_y, valid_domain_list, embedding_meta_data = get_doc2vec_vectors_train_valid_split(
                    df_for_file)
                siamese_lstm_model = None
                if rerank:
                    outputFile = open('../data/output/result2.txt', 'a')
                    #model_path= train_model(train_x, train_y,train_domain_list, embedding_meta_data, fileName)
                    ...
                    for i in range(0, numberOfRowsThatHasDups):
                        ...
                        if(len(dupids) > 0):
                            ...
                            if rerank:
                                ...
                                try:
                                    preds = siamese_lstm_model.predict([test_data_x1, test_data_x2, leaks_test], verbose=0)
                                except Exception:
                                    exceptionCount+=1
                                    print('Booom !!! : '+str(exceptionCount))
                                    continue
                                results = [(x, y, z) for (x, y), z in zip(topn_similar_docs_as_pairs, preds[0])]
